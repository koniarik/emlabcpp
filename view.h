...
class view {
	...
	// Constructor used by the Tags in case public are bloken
	constexpr view(std::pair<Iterator, Iterator> i_range)
	    : begin_(i_range.first), end_(i_range.second) {}

       public:
	// standard public usings for container
	using value_type = typename std::remove_reference_t<decltype(
...
struct detail::is_view_impl<view<Iter, Tag> > : std::true_type {};
