...
struct has_push_back
{
        ...
        template < typename U,
                   typename = decltype( std::declval< U >().push_back( std::declval< typename U::value_type >() ) ) >
        static std::true_type test( int );
...
struct mapped
{
        using type = decltype( std::declval< UnaryFunction >()( *std::begin( std::declval< Container >() ) ) );
};
...
// from here: https://stackoverflow.com/questions/22758291/how-can-i-detect-if-a-type-can-be-streamed-to-an-stdostream
...
{
template < typename Container, typename UnaryFunction >
constexpr std::enable_if_t< !is_std_tuple_v< Container >, void > for_each( Container&& cont, UnaryFunction&& f );
...
namespace detail
{
        ...
        [[nodiscard]] constexpr std::size_t find_if_impl( const std::tuple< Args... >& t,
                                                          UnaryFunction&&              f,
...
template <typename Container,
          typename = std::enable_if_t<std::is_reference_v<Container> ||
                                      is_view_v<Container> > >
...
constexpr U map_range(T input, T from_min, T from_max, U to_min, U to_max) {
    return to_min +
           (to_max - to_min) * (input - from_min) / (from_max / from_min);
}
...
// value 'eps'
template <typename T>
[[nodiscard]] constexpr bool almost_equal(const T& lh, const T& rh,
...
[[nodiscard]] constexpr view<Iterator> tail(Container&& cont, int step = 1) {
    static_assert(
        !std::is_rvalue_reference_v<Container> || is_view_v<Container>,
...
[[nodiscard]] constexpr iterator_of_t<Container> find_if(
    Container&& cont, UnaryFunction&& f = identity()) {
    static_assert(
        !std::is_rvalue_reference_v<Container> || is_view_v<Container>,
...
template <typename Tuple, typename UnaryFunction>
constexpr std::enable_if_t<is_std_tuple_v<Tuple>, void> for_each(
    Tuple&& t, UnaryFunction&& f) {
...
template <typename Container, typename UnaryFunction>
constexpr std::enable_if_t<!is_std_tuple_v<Container>, void> for_each(
    Container&& cont, UnaryFunction&& f) {
...
[[nodiscard]] constexpr min_max<T> min_max_elem(
    const Container& cont, UnaryFunction&& f = identity()) {
    min_max<T> res;
...
template <typename Container, typename UnaryFunction = identity,
          typename T = std::decay_t<mapped_t<Container, UnaryFunction> > >
[[nodiscard]] constexpr T max_elem(const Container& cont,
                                   UnaryFunction&& f = identity()) {
...
[[nodiscard]] constexpr T min_elem(const Container& cont,
                                   UnaryFunction&& f = identity()) {
...
template <typename Container, typename UnaryFunction = identity,
          typename T = std::decay_t<mapped_t<Container, UnaryFunction> > >
[[nodiscard]] constexpr T sum(const Container& cont,
                              UnaryFunction&& f = identity(), T init = {}) {
    for_each(cont, [&](const auto& item) {  //
        init += f(item);
    });
    return init;
...
[[nodiscard]] constexpr T accumulate(const Container& cont, T init,
                                     BinaryFunction&& f) {
    for_each(cont, [&](const auto& item) {  //
...
template <typename Container, typename UnaryFunction = identity,
          typename T = std::decay_t<mapped_t<Container, UnaryFunction> > >
[[nodiscard]] constexpr T avg(const Container& cont,
                              UnaryFunction&& f = identity()) {
    T res{};
    for_each(cont, [&](const auto& item) {  //
        res += f(item);
...
constexpr void for_cross_joint(LhContainer&& lh_cont, RhContainer&& rh_cont,
                               BinaryFunction&& f) {
...
[[nodiscard]] constexpr bool equal(const LhContainer& lh,
...
[[nodiscard]] inline ResultContainer map_f(Container&& cont,
                                           UnaryFunction&& f = identity()) {
...
class static_circular_buffer {
	...
	constexpr auto prev(size_type i) const {
