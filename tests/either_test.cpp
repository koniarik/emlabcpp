...
TEST(Either, convert) {
        ...
        EXPECT_TRUE(eth.convert_left([&](std::string) { return 0; }).is_left());
        EXPECT_FALSE(eth.convert_right([&](int) { return "wololo"; }).is_left());
}

TEST(Either, match) {
        test_either eth{0};

        eth.match([&](std::string) { FAIL(); }, [&](int val) { EXPECT_EQ(val, 0); });
}

TEST(Either, assemble_left_collect_right) {
        using test_either_2 = either<float, int>;

        either<std::tuple<std::string, std::string, float>, static_vector<int, 3>> assemble_either =
            assemble_left_collect_right(test_either{"wolololo"}, test_either{"nope"},
                                        test_either_2{0.f});
